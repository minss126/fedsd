...
def plot_proxy_bars(summary_rows, out_dir):
    ...
    for key, ylabel, filename in metrics:
        ...
        plt.ylabel(ylabel)
        plt.title(f"Fedsd Proxy Alpha Ablation - {ylabel}")
...
def plot_proxy_curves(curve_rows, out_dir):
    ...
    plt.xlabel("Round")
    plt.ylabel("Accuracy (%)")
    plt.title("Fedsd Proxy Alpha Ablation - Accuracy")
    ...
    plt.title("Fedsd Proxy Alpha Ablation - Smoothed Accuracy")
    plt.grid(True, alpha=0.25)
    plt.legend()
    plt.tight_layout()
...
def write_report(path, summaries):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("# Fedsd Proxy Alpha Ablation Report\n\n")
        f.write("## Summary\n\n")
        ...
        f.write("\n## Notes\n\n")
        f.write("- `Fixed alpha` is the same-code control run with no batch-level proxy.\n")
        f.write("- Proxy methods do not use the known Dirichlet beta at training time.\n")
        f.write("- `last_10_acc` is the primary stability metric; `best_acc` is useful but more optimistic.\n")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--log-dir", default="logs_tuning/beta_0.3/fedprox")
    parser.add_argument("--out-dir", default="results_analysis/fedsd_proxy_ablation")
    ...
    if not summaries:
        raise SystemExit(f"No completed proxy ablation pkl/json files found in {args.log_dir}")
    ...
    if curves:
        ...
        plot_proxy_curves(curves, args.out_dir)

    plot_proxy_bars(summaries, args.out_dir)
