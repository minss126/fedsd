...
METHODS = [
    ("fedsd_fixed_alpha_softpilot", "Fixed alpha"),
    ("fedsd_branch_hard_min0p2_softpilot", "Hard branch agreement"),
    ("fedsd_branch_softkl_min0p2", "Soft KL agreement"),
    ("fedsd_branch_js_min0p2", "JS agreement"),
]
...
def plot_curves(curves, out_dir):
    ...
    plt.xlabel("Round")
    plt.ylabel("Accuracy (%)")
    plt.title("Soft Branch Proxy Pilot - Accuracy")
    ...
    plt.title("Soft Branch Proxy Pilot - Smoothed Accuracy")
    plt.grid(True, alpha=0.25)
    plt.legend()
    plt.tight_layout()
...
def plot_bars(summaries, out_dir):
    ...
    for key, ylabel, filename in metrics:
        ...
        plt.ylabel(ylabel)
        plt.title(f"Soft Branch Proxy Pilot - {ylabel}")
...
def write_report(path, summaries):
    fixed = next((row for row in summaries if row["method"] == "fedsd_fixed_alpha_softpilot"), None)
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("# Soft Branch Proxy Pilot Report\n\n")
        f.write("## Summary\n\n")
        f.write("| Method | Proxy | Last-10 Acc | Best Acc | Best Round | Final Acc | Delta Last-10 vs Fixed | Last-10 Loss | Last-10 ECE |\n")
        f.write("|---|---|---:|---:|---:|---:|---:|---:|---:|\n")
        for row in summaries:
            ...
            f.write(
                f"| {row['label']} | {row.get('byot_alpha_proxy', 'none')} | "
        ...
        f.write("\n## Notes\n\n")
        f.write("- This pilot uses a same-code fixed control run.\n")
        f.write("- Hard branch agreement uses argmax match.\n")
        f.write("- Soft KL and JS use distribution-level teacher/branch consistency.\n")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--log-dir", default="logs_prev/logs_tuning/beta_0.3/fedprox")
    parser.add_argument("--out-dir", default="results_analysis/soft_branch_proxy_pilot")
    ...
    if not summaries:
        raise SystemExit(f"No soft branch proxy pilot runs found in {args.log_dir}")
