...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
    ...
    echo "  METHODS_OVERRIDE=\"gate_soft_tau0p75_temp0p05 gate_soft_tau0p80_temp0p05 gate_hard_tau0p75\""
    echo "  LOG_ROOT=\"logs/lambda/selective/logs_lambda_granularity_gate\""
    echo "  USE_WANDB=0"
    exit 0
fi

# Granularity-gated lambda control.
#
# It interpolates between:
#   sample-wise: lambda_sample_{k,i} = lambda * p_T(y_i | x_i)
#   client-wise: lambda_client_k     = lambda * r_k * b_k^2
#
# Final:
#   lambda_{k,i} = (1-g_k) * lambda_sample_{k,i} + g_k * lambda_client_k
#   g_k = sigmoid((tau - b_k) / T), or a hard threshold version.
...
LOG_ROOT="${LOG_ROOT:-logs/lambda/selective/logs_lambda_granularity_gate}"
SKIP_EXISTING="${SKIP_EXISTING:-1}"
EXTRA_FLAGS="${EXTRA_FLAGS:---log_client_group_lambda}"

ENVS=(${ENVS_OVERRIDE:-iid beta_0.5 beta_0.3 beta_0.1})
METHODS=(${METHODS_OVERRIDE:-gate_soft_tau0p75_temp0p05 gate_soft_tau0p80_temp0p05 gate_hard_tau0p75})
...
method_flags() {
    case "$1" in
        gate_soft_tau0p75_temp0p05)
            echo "$(base_gate_flags) --byot_lambda_gate_mode soft --byot_lambda_gate_tau 0.75 --byot_lambda_gate_temperature 0.05"
            ;;
        gate_soft_tau0p80_temp0p05)
            echo "$(base_gate_flags) --byot_lambda_gate_mode soft --byot_lambda_gate_tau 0.80 --byot_lambda_gate_temperature 0.05"
            ;;
        gate_soft_tau0p75_temp0p10)
            echo "$(base_gate_flags) --byot_lambda_gate_mode soft --byot_lambda_gate_tau 0.75 --byot_lambda_gate_temperature 0.10"
            ;;
        gate_hard_tau0p75)
            echo "$(base_gate_flags) --byot_lambda_gate_mode hard --byot_lambda_gate_tau 0.75"
            ;;
        *)
...
wait "${pids[@]}"
echo "Lambda granularity gate experiments complete (${job_count} jobs)"
