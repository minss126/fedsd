...
# Completes the missing conditions for MobileNet and MOON on a two-GPU server.
# Phase 1 finishes the requested three-method table at beta={0.5,0.1}; phase
# 2 then adds IID and beta=0.3.

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
cd "$REPO_ROOT"

export GPUS_OVERRIDE="${GPUS_OVERRIDE:-0 1}"
export EXTENSIONS_OVERRIDE="mobilenet moon"
...
echo "2-GPU extension completion pipeline complete."
