...
# FedAvg version of the update-space client drift sweep.
# This checks whether gradient-level drift amplification is reflected in realized local updates
# when FedProx regularization is removed.
#
# Results are isolated under logs/drift/logs_drift_fedavg/.
#
# wandb is enabled by default for experiment tracking.
# Disable if needed:
#   USE_WANDB=0 ./scripts/experiments/drift/run_fedsd_drift_fedavg_sweep.sh

GPUS=(0 1 2 3)
NUM_GPUS=${#GPUS[@]}
JOB_COUNT=0

LOG_ROOT="${LOG_ROOT:-logs/drift/logs_drift_fedavg}"
...
echo "========== FedSD Update Drift Sweep: FedAvg =========="
...
wait
echo "✅ FedAvg update drift sweep 완료"
