...
# Same complete, resumable experiment suite as the 4-GPU launcher.  Jobs are
# automatically distributed sequentially over two GPUs by each stage script.

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
cd "$REPO_ROOT"

GPU_IDS="${GPUS_OVERRIDE:-0 1}"
export USE_WANDB="${USE_WANDB:-0}"

echo "========== Complete Branch-Mechanism Suite (2 GPU) =========="
echo "gpus=${GPU_IDS}"
